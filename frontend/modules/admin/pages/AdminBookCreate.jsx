...
const AdminBookCreate = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    const downloadUrl = sanitizeUrl(formData.downloadUrl) || sanitizeUrl(formData.previewUrl) || sanitizeUrl(formData.coverImage) || "";

    if (!downloadUrl) {
      toast.error("Please provide a download URL, preview URL, or cover image URL for the ebook file.");
      return;
    }
    ...
    try {
      const created = await createEbook(payload);
        ...
        <section className="layout-container space-y-8">
          ...
          <motion.form
            ...
            <section className="grid gap-4 md:grid-cols-2">
              ...
              <div className="space-y-1.5 md:col-span-2">
                <label htmlFor="downloadUrl" className="text-xs font-semibold uppercase tracking-[0.3em] text-[#F5D26A]/80">
                  Download URL*
                </label>
                <input
                  id="downloadUrl"
                ...
                />
                <p className="text-[11px] text-slate-400">
                  Required: Direct URL to the PDF file. Can also use preview URL or cover image URL if they point to the PDF.
                </p>
              </div>
