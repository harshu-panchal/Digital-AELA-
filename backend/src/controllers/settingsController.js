    ...
    for (const setting of settings) {
      try {
        ...
        if (validatedValue === null) {
          errors.push({
            key,
            error: `Invalid value type for ${type || "string"}`,
          });
          continue;
        }
        ...
        const isContactInfo =
          key === "site.contact.email" || key === "site.contact.phone";
...
export const updateSetting = async (req, res, next) => {
  try {
    const { key } = req.params;
    const { value, category, type, label, description, isPublic, isEncrypted } =
      req.body;
    const { userId } = req.auth;
    ...
    // Automatically set isPublic to true for social media settings and contact info
    const isContactInfo =
      setting.key === "site.contact.email" ||
      setting.key === "site.contact.phone";
    if (isPublic !== undefined) {
      setting.isPublic = isPublic;
    } else if (
      setting.category === "social" ||
      setting.key.startsWith("social.") ||
      isContactInfo
    ) {
  ...
  try {
    ...
    const defaultSettings = [
      ...
      // Email Settings
      {
        ...
        description: "Name shown in email sender",
      },
      {
        key: "email.from.address",
        value: "noreply@digitalaela.com",
        category: "email",
        type: "string",
      ...
      {
        key: "payment.gateway.stripe.enabled",
        value: false,
        category: "payment",
        type: "boolean",
      ...
      {
        key: "payment.gateway.stripe.publicKey",
        value: "",
        category: "payment",
        type: "string",
      ...
      {
        key: "payment.gateway.stripe.secretKey",
        value: "",
        category: "payment",
        type: "string",
      ...
      {
        key: "payment.gateway.paypal.enabled",
        value: false,
        category: "payment",
        type: "boolean",
      ...
      {
        key: "payment.gateway.paypal.clientId",
        value: "",
        category: "payment",
        type: "string",
      ...
      {
        key: "payment.gateway.paypal.secret",
        value: "",
        category: "payment",
        type: "string",
        label: "PayPal Secret",
      ...
      },
      {
        ...
        description: "Razorpay key secret (private key)",
        isEncrypted: true,
      },
      {
      ...
      {
        key: "features.jobs.enabled",
        value: true,
        category: "features",
        type: "boolean",
        label: "Jobs Enabled",
        description: "Enable job portal functionality",
        isPublic: true,
      },
      {
      ...
      {
        key: "features.ebooks.enabled",
        value: true,
        category: "features",
        type: "boolean",
        label: "Ebooks Enabled",
        description: "Enable ebook functionality",
        isPublic: true,
      },
      {
        ...
        category: "features",
        type: "boolean",
        label: "Quizzes Enabled",
        description: "Enable quiz functionality",
        isPublic: true,
      },
      {
        key: "features.points.enabled",
        value: true,
        category: "features",
        type: "boolean",
      ...
      {
        key: "features.messaging.enabled",
        value: true,
        category: "features",
        type: "boolean",
        label: "Messaging Enabled",
        description: "Enable messaging functionality",
        isPublic: true,
      },
      ...
      {
        key: "seo.meta.title",
        value: "Digital AELA - Learn, Earn, and Grow",
        category: "seo",
        type: "string",
        label: "Meta Title",
      ...
      },
      {
        ...
        value: "Digital AELA platform for learning and career growth",
        category: "seo",
        type: "string",
      ...
      },
      {
        ...
        value: "learning, courses, jobs, career",
        category: "seo",
        type: "string",
        label: "Meta Keywords",
    ...
    for (const settingData of defaultSettings) {
      try {
        ...
        if (!existing) {
          ...
          if (
            settingData.category === "social" ||
            settingData.key?.startsWith("social.")
          ) {
          ...
          if (
            settingData.category === "features" ||
            settingData.key?.startsWith("features.")
          ) {
...
export const verifyFinancialPassword = async (req, res, next) => {
  try {
    ...
    const isValid = await bcrypt.compare(
      password,
      financialPasswordSetting.value
    );
  ...
  try {
    ...
    await Settings.findOneAndUpdate(
      { key: "financial.password" },
      {
        ...
        description:
          "Password required to access financial pages (payments, expenses, financial dashboard)",
        isEncrypted: true,
        updatedBy: userId,
      },
      {
...
export const requestFinancialPasswordReset = async (req, res, next) => {
  try {
    ...
    try {
      await sendFinancialPasswordResetEmail(
        recipientEmail,
        token,
        adminUser.fullName
      );
      ...
      console.error(
        "Failed to send financial password reset email:",
        emailError
      );
    ...
    return res.status(200).json({
      success: true,
      message:
        "Financial password reset email has been sent to info.digitalaela@gmail.com",
    });
  } catch (error) {
    return next(error);
  }
...
export const resetFinancialPasswordWithToken = async (req, res, next) => {
  try {
    ...
    // Validate password strength (minimum 12 characters + complexity)
    const passwordRegex =
      /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{12,}$/;
